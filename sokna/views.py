...
def register(req):
    ...
    if req.method == 'POST':
        ...
        if form.is_valid():
            instance = form.save()
            messages.success(req, '<i class="fa fa-fw fa-check"></i> %s' %
...
def follow(req):
    ...
    if req.GET.get('CIN', None):
        ...
        if form.is_valid():
            ...
            try:
                SR = SoknaRequest.objects.get(
                    id=form.cleaned_data['submission_code'], CIN=form.cleaned_data['CIN'])
            except (SoknaRequest.DoesNotExist, ValueError):
